...
/**
 * Options for coordinate sheet PDF export.
 */
export interface CoordinateSheetPdfOptions {
...
/**
 * Difficulty color map (RGB) matching CoordinateSheetView badge colors.
 */
const DIFFICULTY_COLORS: Record<string, { r: number; g: number; b: number }> = {
  easy: { r: 34, g: 139, b: 34 },       // green
  moderate: { r: 202, g: 138, b: 4 },    // yellow/amber
  hard: { r: 220, g: 38, b: 38 },        // red
};
...
export async function exportToCoordinateSheetPdf(
  ...
  tempoMap?: TempoMap,
  options?: CoordinateSheetPdfOptions,
): Promise<Blob> {
  ...
  // Track total pages for "Page X of Y" footer — rendered after all content
  let totalPages = 1;
  ...
  for (let idx = 0; idx < entries.length; idx++) {
    ...
    // Difficulty coloring — tint the row text for moderate/hard
    const difficulty = entry.stepToNext?.difficulty;
    ...
    // Reset text color after difficulty coloring
    if (showDifficulty && difficulty && DIFFICULTY_COLORS[difficulty]) {
      doc.setTextColor(0, 0, 0);
    }
