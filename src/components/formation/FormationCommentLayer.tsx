/**
 * FormationCommentLayer — DOM overlay for Figma-style comment annotations
 * on the formation canvas.
 *
 * Comments are anchored to (x, y) positions in normalized coordinates (0-100).
 * Supports threading, reactions, and resolution.
 */

import React, { useState, useCallback, useRef, useEffect, useMemo } from 'react';
import { MessageCircle, Check, Trash2, X } from 'lucide-react';

const REACTION_EMOJIS = ['\u{1F44D}', '\u2705', '\u{1F440}', '\u2764\uFE0F'] as const;
...
export interface FormationComment {
  ...
  performerId?: string; // optional: anchored to a specific performer
  setId?: string; // optional: anchored to a specific set/keyframe
  userId: string;
  username: string;
  color: string;
...
export interface FormationCommentLayerProps {
  ...
  activeTool: string; // 'comment' when comment mode is active
  onAddComment: (x: number, y: number, text: string, performerId?: string) => void;
...
export const FormationCommentLayer = React.memo(function FormationCommentLayer({
  comments,
  ...
  onDeleteComment,
  onToggleReaction,
}: FormationCommentLayerProps) {
  // stageWidth, stageHeight, canvasWidth, canvasHeight are available for
  ...
  // Group comments: top-level (resolved shown as faded) + their replies
  ...
  const repliesMap = useMemo(() => {
    const map = new Map<string, FormationComment[]>();
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      // Only trigger on direct clicks on the overlay, not on child elements
      ...
      // Convert pixel position to normalized 0-100 coordinates
      const normX = (clickX / rect.width) * 100;
      const normY = (clickY / rect.height) * 100;

      setAddingAt({ x: normX, y: normY });
      setNewCommentText('');
  ...
  const handleSubmitComment = useCallback(() => {
    if (!addingAt || !newCommentText.trim()) return;
    onAddComment(addingAt.x, addingAt.y, newCommentText.trim());
    setAddingAt(null);
    setNewCommentText('');
  }, [addingAt, newCommentText, onAddComment]);
  ...
  return (
    ...
    >
          ...
          >
            ...
            <div className="relative">
              <button
                ...
                className={`w-6 h-6 rounded-full flex items-center justify-center hover:scale-125 transition-transform shadow-sm ${
                  isResolved ? 'opacity-40' : ''
                }`}
                style={{ backgroundColor: isResolved ? '#9ca3af' : comment.color }}
                title={`${comment.username}: ${comment.text}`}
              >
                <MessageCircle className="w-3.5 h-3.5 text-white" aria-hidden="true" />
              </button>
              {replyCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-indigo-600 text-white text-[9px] flex items-center justify-center font-medium">
                  {replyCount}
                </span>
              )}
            </div>
            ...
            {/* Thread popover */}
            {isOpen && (
              <div className="absolute top-8 left-1/2 -translate-x-1/2 z-50 w-72 bg-white rounded-lg shadow-lg border border-neutral-200 p-3 text-xs max-h-80 overflow-y-auto">
      ...
      {addingAt !== null && (
        ...
        >
          <div className="w-60 bg-white rounded-lg shadow-lg border border-neutral-200 p-3">
