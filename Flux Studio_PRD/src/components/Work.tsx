        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {projects.map((project, index) => (
            <div key={index} className={`group cursor-pointer ${project.featured ? 'lg:col-span-2' : ''}`}>
              <div className={`relative overflow-hidden rounded-2xl bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/20 transition-all duration-500 ${project.featured ? 'md:flex md:h-96' : 'h-80'}`}>
                {/* Image */}
                <div className={`relative overflow-hidden ${project.featured ? 'md:w-3/5' : 'h-1/2'}`}>
                  ...
                  <div className="absolute top-4 left-4">
                    <Badge className="bg-gradient-to-r from-pink-500 via-purple-600 to-cyan-400 text-black font-semibold">
                      {project.type}
                    </Badge>
                  </div>
                </div>
                ...
                {/* Hover overlay */}
                <div className="absolute inset-0 bg-gradient-to-r from-pink-500/10 via-purple-600/10 to-cyan-400/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              </div>
            </div>
          ))}
        </div>
