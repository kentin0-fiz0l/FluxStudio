...
class OAuthManager {
  ...
  initializeProviders() {
    ...
    log.info(`OAuth Manager initialized with ${this.providers.size} providers`);
  }
  ...
  async handleCallback(provider, code, stateToken) {
    ...
    try {
      ...
      log.error(`${provider} token exchange error`, { details: error.response?.data || error.message });
    ...
    try {
      ...
      log.warn(`Failed to fetch ${provider} user info`, { message: error.message });
    }
    ...
    log.info(`OAuth tokens stored for user ${userId} (${provider})`);
  ...
  async refreshAccessToken(userId, provider, refreshToken) {
    ...
    try {
      ...
      log.error(`${provider} token refresh error`, { details: error.response?.data || error.message });
    ...
    log.info(`Access token refreshed for user ${userId} (${provider})`);
  ...
  async disconnectIntegration(userId, provider) {
    ...
    log.info(`${provider} integration disconnected for user ${userId}`);
  }
  ...
  async fetchFigmaUserInfo(accessToken) {
    try {
      ...
      log.error('Figma user info fetch error', { message: error.message });
      return {};
    }
  }
  ...
  async fetchSlackUserInfo(accessToken) {
    try {
      ...
      log.error('Slack user info fetch error', { message: error.message });
      return {};
    }
  }
  ...
  async fetchGitHubUserInfo(accessToken) {
    try {
      ...
      log.error('GitHub user info fetch error', { message: error.message });
      return {};
    }
  }
  ...
  async fetchGoogleUserInfo(accessToken) {
    try {
      ...
      log.error('Google user info fetch error', { message: error.message });
      return {};
    }
  }
  ...
  async fetchDropboxUserInfo(accessToken) {
    try {
      ...
      log.error('Dropbox user info fetch error', { message: error.message });
      return {};
    }
  }
  ...
  async fetchOneDriveUserInfo(accessToken) {
    try {
      ...
      log.error('OneDrive user info fetch error', { message: error.message });
      return {};
    }
  }
  ...
  encryptToken(token) {
    ...
    if (!encryptionKey || encryptionKey.length < 32) {
      log.warn('OAuth encryption key not set or too short. Tokens will be stored in plaintext!');
      log.warn('Set OAUTH_ENCRYPTION_KEY environment variable (min 32 characters)');
    ...
    try {
      ...
      log.error('Token encryption error', { message: error.message });
  ...
  decryptToken(encryptedToken) {
    ...
    const encryptionKey = process.env.OAUTH_ENCRYPTION_KEY;
    if (!encryptionKey || encryptionKey.length < 32) {
      log.error('Cannot decrypt token: OAUTH_ENCRYPTION_KEY not set');
      return null;
    }
    ...
    try {
      ...
      log.error('Token decryption error', { message: error.message });
