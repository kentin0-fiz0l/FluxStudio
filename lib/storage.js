...
const { S3Client, PutObjectCommand, DeleteObjectCommand, GetObjectCommand, HeadObjectCommand, CopyObjectCommand, ListObjectsV2Command } = require('@aws-sdk/client-s3');
...
const s3Breaker = createCircuitBreaker({
  name: 's3-api',
  failureThreshold: 5,
...
class StorageAdapter {
  ...
  /**
   * Validate that a storage key does not escape the storage root via path traversal.
   * Returns the resolved absolute path.
   */
  _safePath(storageKey) {
    const resolvedRoot = path.resolve(this.storageRoot);
    const resolvedPath = path.resolve(this.storageRoot, storageKey);
  ...
  async getFileStream(storageKey) {
    ...
    const fullPath = this._safePath(storageKey);
  ...
  getFullPath(storageKey) {
    return this._safePath(storageKey);
  }
  ...
  async copyFile(sourceKey, destKey) {
    ...
    const sourcePath = this._safePath(sourceKey);
    const destPath = this._safePath(destKey);
...
class FileStorage {
  ...
  async uploadToS3(fileName, buffer, mimeType, isPublic = false) {
    ...
    await s3Breaker.execute(() => new Upload({ client: s3, params }).done());
    return key;
  }
  ...
  async deleteFromS3(key) {
    await s3Breaker.execute(() => s3.send(new DeleteObjectCommand({
      Bucket: BUCKET_NAME,
      Key: key,
    })));
  }
  ...
  async getSignedUrl(filePath, expiresIn = 3600) {
    if (process.env.NODE_ENV === 'production' && process.env.AWS_ACCESS_KEY_ID) {
      return await s3Breaker.execute(() => getSignedUrl(s3, new GetObjectCommand({
        Bucket: BUCKET_NAME,
        Key: filePath,
      }), { expiresIn }));
    } else {
...
module.exports.StorageAdapter = StorageAdapter;
module.exports.s3Breaker = s3Breaker;
