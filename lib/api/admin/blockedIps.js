...
const log = createLogger('AdminBlockedIps');
...
router.get('/blocked-ips', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error listing blocked IPs', error);
    res.status(500).json({
      success: false,
...
router.get('/blocked-ips/:ip', adminAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      log.error('Error reading security events', error);
    }
    ...
    log.error('Error getting IP details', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.post('/blocked-ips/:ip/whitelist', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error whitelisting IP', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/blocked-ips/stats', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting blocked IPs stats', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
