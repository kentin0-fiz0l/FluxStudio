...
const log = createLogger('AdminMaintenance');
...
router.post('/cleanup', adminAuth, async (req, res) => {
  try {
    ...
    log.info('Starting cleanup', { dryRun });
    ...
    log.error('Error running cleanup', error);
    res.status(500).json({
      success: false,
...
router.get('/cleanup/status', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting cleanup status', error);
    res.status(500).json({
      success: false,
...
router.post('/cleanup/schedule', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error updating cleanup schedule', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/performance/summary', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting performance summary', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/performance/endpoints', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting endpoint performance', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/health', adminAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      log.debug('PM2 status not available', { error: error.message });
    }
