...
const log = createLogger('AdminTokens');
...
router.get('/tokens/stats', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting token stats', error);
    res.status(500).json({
      success: false,
...
router.get('/tokens/search', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error searching tokens', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/tokens/:tokenId', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting token details', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/tokens/analytics', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting token analytics', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
