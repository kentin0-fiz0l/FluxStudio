...
const log = createLogger('AdminSecurity');
...
async function loadEvents() {
  try {
    ...
    log.error('Error loading security events', error);
    return [];
  }
}
...
router.get('/events', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error listing security events', error);
    res.status(500).json({
      success: false,
...
router.get('/events/:eventId', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting event details', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/events/export', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error exporting events', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
...
router.get('/timeline', adminAuth, async (req, res) => {
  try {
    ...
    log.error('Error getting timeline', error);
    res.status(500).json({
      success: false,
      message: 'Internal server error',
      error: error.message
