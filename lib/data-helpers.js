...
const log = createLogger('DataHelpers');
...
async function saveProjects(projects) {
  ...
  // Invalidate project caches when projects are saved
  if (cacheInitialized && Array.isArray(projects)) {
...
async function createMessage(messageData) {
  let newMessage;
  if (messagingAdapter) {
    newMessage = await performanceMonitor.monitorDatabaseQuery('createMessage', () => messagingAdapter.createMessage(messageData));
  } else {
  ...
  if (cacheInitialized && newMessage && messageData.conversationId) {
    await cache.invalidate.conversation(messageData.conversationId);
  }
...
async function saveChannels(channels) {
  ...
  // Invalidate conversation caches when channels are saved
  if (cacheInitialized && Array.isArray(channels)) {
    for (const channel of channels) {
      if (channel.id) {
        await cache.invalidate.conversation(channel.id);
      }
    }
  }
}
