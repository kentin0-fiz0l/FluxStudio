...
const log = createLogger('RefreshTokenRoutes');
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    // Return new token pair wrapped in standard response format
    res.json({
      success: true,
      data: {
...
router.post('/logout-all', authenticateToken, async (req, res) => {
  try {
    ...
    log.error('Error logging out from all devices', error);
    res.status(500).json({
      error: 'Internal server error',
...
router.get('/token-info', authenticateToken, (req, res) => {
  try {
    ...
    log.error('Error getting token info', error);
    res.status(500).json({
      error: 'Internal server error',
