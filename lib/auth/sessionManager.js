...
async function createSession(userId, tokenId, req) {
  ...
  try {
    await query(
      `INSERT INTO active_sessions (id, user_id, token_id, device_info, ip_address)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (token_id) DO UPDATE SET last_active_at = NOW()`,
      [generateId(), userId, tokenId, JSON.stringify(deviceInfo), ip]
    );
  } catch (error) {
...
async function enforceSessionLimit(userId, maxSessions = 5) {
  try {
    ...
    if (result.rows.length > maxSessions) {
      ...
      await query(
        `DELETE FROM active_sessions WHERE id = ANY($1::text[])`,
        [ids]
      );
    }
  } catch (error) {
    log.error('Failed to enforce limit', error);
  }
}
