...
class TokenCleanupService {
  ...
  async cleanupRevokedTokens() {
    try {
      ...
      if (deletedCount > 0) {
        await this.saveTokens(validTokens);
        console.log(`🧹 Cleaned up ${deletedCount} old revoked tokens`);
  ...
  async cleanupOrphanedSessions() {
    try {
      ...
      if (deletedCount > 0) {
        ...
        console.log(`🧹 Cleaned up ${deletedCount} orphaned sessions`);
  ...
  async cleanupCacheEntries() {
    try {
      ...
      console.log('✅ Redis cache TTL cleanup handled automatically');
  ...
  async archiveOldSecurityEvents() {
    try {
      ...
      try {
        ...
        console.log('ℹ️  No security events file to archive');
        return 0;
      }
      ...
      if (oldEvents.length > 0) {
        ...
        console.log(`📦 Archived ${oldEvents.length} old security events`);
      ...
      console.error('Error archiving security events:', error);
      return 0;
    }
  }
