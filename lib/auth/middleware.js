...
async function authenticateToken(req, res, next) {
  try {
    ...
    // Check if access token has been revoked (Redis blacklist)
    if (decoded.jti) {
      const revoked = await tokenService.isTokenRevoked(decoded.jti);
      if (revoked) {
        return res.status(401).json({
          error: 'Unauthorized',
    ...
    // Phase 5: Check for expired trial — revert to free plan on login
    if (_dbQuery && decoded.id) {
      try {
        const trialResult = await _dbQuery(
          'SELECT plan_id, trial_ends_at FROM users WHERE id = $1',
          [decoded.id]
        );
        const row = trialResult.rows[0];
        if (row && row.plan_id === 'pro' && row.trial_ends_at && new Date(row.trial_ends_at) < new Date()) {
          // Check if user has an active paid subscription before downgrading
          const subResult = await _dbQuery(
            "SELECT id FROM subscriptions WHERE user_id = $1 AND status = 'active' LIMIT 1",
            [decoded.id]
          );
          if (subResult.rows.length === 0) {
            await _dbQuery(
              "UPDATE users SET plan_id = 'free' WHERE id = $1",
              [decoded.id]
            );
            log.info('Expired trial reverted to free', { userId: decoded.id });
          }
        }
      } catch (trialErr) {
        // Non-blocking — don't fail auth if trial check fails
        log.warn('Trial expiry check failed', trialErr.message);
      }
    }
...
function rateLimitByUser(maxRequests = 100, windowMs = 60000) {
  ...
  try {
    ...
    log.warn('Redis not available for rate limiting, using in-memory fallback');
  }
