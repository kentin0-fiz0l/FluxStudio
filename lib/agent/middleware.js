...
const log = createLogger('AgentMiddleware');
...
async function incrementRequestCount(userId) {
  try {
    ...
    log.error('incrementRequestCount error', error);
  }
}
...
function agentPermissions(requiredScope) {
  return async (req, res, next) => {
    try {
      ...
      log.error('agentPermissions error', error);
      res.status(500).json({
    ...
    res.send = function(body) {
      ...
      setImmediate(async () => {
        try {
          ...
          log.error('auditLog error', error);
        }
      });
...
function agentRateLimit(maxRequests = 30, windowMs = 60000) {
  ...
  return async (req, res, next) => {
    try {
      ...
      log.error('agentRateLimit error', error);
      next(); // Fail open
    }
  };
}
