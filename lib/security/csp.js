...
function buildCSPHeader(options = {}) {
  ...
  // Note: Some UI libraries may add inline styles. If issues occur,
  // add specific hashes or use 'unsafe-hashes' for specific patterns.
...
function cspReportHandler(req, res) {
  ...
  // Extract CSP violation details (handle both report formats)
  ...
  const violationDetails = {
    ...
    scriptSample: violation['script-sample'] || violation.scriptSample,
  ...
  console.warn('[CSP Violation]', JSON.stringify(violationDetails, null, 2));

  // Track violation patterns for analysis
  trackViolationPattern(violationDetails);
  ...
  if (process.env.SENTRY_DSN) {
    try {
      ...
      Sentry.captureMessage('CSP Violation', {
        ...
        tags: {
          type: 'csp_violation',
          directive: violationDetails.effectiveDirective || violationDetails.violatedDirective
        },
        extra: violationDetails
      });
    } catch (err) {
      // Sentry not available or not configured
      console.debug('Sentry not available for CSP reporting:', err.message);
    }
  }

  // Alert on suspicious patterns (potential XSS attempts)
  if (isSuspiciousViolation(violationDetails)) {
    console.error('[SECURITY ALERT] Suspicious CSP violation detected:', violationDetails);

    // Could trigger alerts here (email, Slack, PagerDuty, etc.)
    if (process.env.SECURITY_WEBHOOK_URL) {
      sendSecurityAlert(violationDetails).catch(err => {
...
/**
 * Track violation patterns for analysis
 * Helps identify recurring issues and potential attacks
 */
const violationPatterns = new Map();

function trackViolationPattern(violation) {
  const key = `${violation.effectiveDirective}:${violation.blockedUri}`;
  const pattern = violationPatterns.get(key) || { count: 0, firstSeen: Date.now(), lastSeen: null };

  pattern.count++;
  pattern.lastSeen = Date.now();

  violationPatterns.set(key, pattern);

  // Clean up old patterns (older than 1 hour)
  const oneHourAgo = Date.now() - 3600000;
  for (const [k, v] of violationPatterns.entries()) {
    if (v.lastSeen < oneHourAgo) {
      violationPatterns.delete(k);
    }
  }

  // Log if pattern is repeating frequently (potential attack)
  if (pattern.count > 10 && (Date.now() - pattern.firstSeen) < 60000) {
    console.warn(`[CSP] High frequency violation pattern detected: ${key} (${pattern.count} times in < 1 minute)`);
  }
}

/**
 * Check if violation appears suspicious (potential attack)
 */
function isSuspiciousViolation(violation) {
  const suspiciousPatterns = [
    ...
    /^https?:\/\/(?!localhost|fluxstudio\.art|accounts\.google\.com|fonts\.googleapis\.com)/i
  ];
 ...
 */
async function sendSecurityAlert(violation) {
  ...
  await axios.post(webhookUrl, {
    type: 'csp_violation',
    severity: 'warning',
    title: 'Suspicious CSP Violation Detected',
    description: `A potentially malicious CSP violation was detected`,
    details: violation,
    timestamp: new Date().toISOString()
  }, {
    timeout: 5000,
...
function getViolationStats() {
  const stats = {
    totalPatterns: violationPatterns.size,
    topViolations: []
  };

  // Sort by count and get top 10
  const sorted = [...violationPatterns.entries()]
    .sort((a, b) => b[1].count - a[1].count)
    .slice(0, 10);

  stats.topViolations = sorted.map(([key, data]) => ({
    pattern: key,
    count: data.count,
