...
class AnomalyDetector {
  ...
  async checkFailedLoginRate(email, ipAddress) {
    try {
      ...
      log.error('Error checking failed login rate', error);
  ...
  async resetFailedLoginCounter(email, ipAddress) {
    try {
      ...
      log.error('Error resetting failed login counter', error);
    }
  }
  ...
  async checkMultipleDevices(userId, sessions) {
    try {
      ...
      log.error('Error checking multiple devices', error);
      return false;
    }
  }
  ...
  async checkTokenRefreshRate(userId, tokenId) {
    try {
      ...
      log.error('Error checking token refresh rate', error);
      return false;
    }
  }
  ...
  async checkRequestRate(ipAddress, endpoint) {
    try {
      ...
      log.error('Error checking request rate', error);
      return false;
    }
  }
  ...
  async checkAccountTakeover(userId, currentIp, previousIp, currentLocation, previousLocation) {
    try {
      ...
      log.error('Error checking account takeover', error);
      return false;
    }
  }
  ...
  async incrementCounter(key, ttl) {
    try {
      ...
      log.error('Error incrementing counter', error);
      return 0;
    }
  }
  ...
  async reportAnomaly(anomaly) {
    try {
      ...
      // Log to console
      log.warn('Security anomaly detected', { type: anomaly.type, severity: anomaly.severity });
      ...
      if (anomaly.ipAddress) {
        try {
          ...
          log.error('Error updating IP reputation', error);
        }
      }
      ...
      log.error('Error reporting anomaly', error);
    }
  }
  ...
  async getUserAnomalyStats(userId, days = 30) {
    try {
      ...
      log.error('Error getting user anomaly stats', error);
      return null;
    }
  }
  ...
  async blockIpAddress(ipAddress, duration = 3600, reason = 'suspicious_activity') {
    try {
      ...
      log.warn('IP blocked', { ipAddress, duration, reason });
      ...
      log.error('Error blocking IP address', error);
    }
  }
  ...
  async isIpBlocked(ipAddress) {
    try {
      ...
      log.error('Error checking IP block', error);
