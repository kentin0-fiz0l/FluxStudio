...
class IPReputationSystem {
  ...
  async getWorstIPs(limit = 10) {
    try {
      ...
      log.warn('getWorstIPs: Feature requires Redis SCAN implementation');
      return [];
    } catch (error) {
  ...
  async getBestIPs(limit = 10) {
    try {
      ...
      log.warn('getBestIPs: Feature requires Redis SCAN implementation');
      return [];
    } catch (error) {
  ...
  async applyDailyDecay() {
    try {
      ...
      log.warn('Daily decay: Requires Redis SCAN implementation for production');
