...
const { getModelForTask, getMaxTokensForTask } = require('../ai/config');
const { extractJSON } = require('../ai/response-handlers');
...
Be specific, actionable, and constructive in your feedback.`;
    ...
    const response = await client.messages.create({
      model: getModelForTask('design-feedback'),
      max_tokens: 2000,
      messages: [{
        role: 'user',
        content: [
...
Respond with ONLY valid JSON:
{
  ...
  "description": "brief description of why these colors work together"
    ...
    const paletteModel = getModelForTask('palette');
    const response = await client.messages.create({
      model: paletteModel,
      max_tokens: getMaxTokensForTask('palette'),
...
Respond with ONLY valid JSON:
{
  ...
  "reformulated": "cleaner version of the query for full-text search"
    ...
    const searchModel = getModelForTask('search');
    const response = await client.messages.create({
      model: searchModel,
...
async function streamChat(userId, messages, context = {}) {
  try {
    ...
    const stream = await client.messages.stream({
      model: context.model || getModelForTask('chat'),
