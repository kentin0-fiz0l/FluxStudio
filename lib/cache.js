...
async function initializeCache() {
  try {
    ...
    if (redisClient && isConnected) {
      log.info('Redis cache already connected');
      return redisClient;
    }
    ...
    log.error('Redis initialization failed', { message: err.message });
    log.warn('Continuing without cache (degraded performance)');
    isConnected = false;
    return null;
  }
}
...
async function get(key) {
  ...
  try {
    ...
    log.error(`Cache GET error for key "${key}"`, { message: err.message });
    return null;
  }
}
...
async function set(key, value, ttl = TTL.MEDIUM) {
  ...
  try {
    ...
    log.error(`Cache SET error for key "${key}"`, { message: err.message });
    return false;
  }
}
...
async function del(key) {
  ...
  try {
    ...
    log.error(`Cache DEL error for key "${key}"`, { message: err.message });
    return false;
  }
}
...
async function deletePattern(pattern) {
  ...
  try {
    ...
    log.error(`Cache DELETE PATTERN error for pattern "${pattern}"`, { message: err.message });
    return 0;
  }
}
...
async function exists(key) {
  ...
  try {
    ...
    log.error(`Cache EXISTS error for key "${key}"`, { message: err.message });
    return false;
  }
}
...
async function incr(key, increment = 1) {
  ...
  try {
    ...
    log.error(`Cache INCR error for key "${key}"`, { message: err.message });
    return 0;
  }
}
...
async function expire(key, ttl) {
  ...
  try {
    ...
    log.error(`Cache EXPIRE error for key "${key}"`, { message: err.message });
    return false;
  }
}
...
async function getOrSet(key, fetchFn, ttl = TTL.MEDIUM) {
  ...
  try {
    ...
    log.error(`Cache getOrSet error for key "${key}"`, { message: err.message });
    throw err;
  }
}
...
async function flushAll() {
  ...
  try {
    ...
    log.warn('Cache flushed (all keys deleted)');
    return true;
  } catch (err) {
    log.error('Cache flush error', { message: err.message });
    return false;
  }
}
...
async function closeCache() {
  if (redisClient) {
    try {
      ...
      log.error('Error closing Redis connection', { message: err.message });
    }
  }
}
...
async function getAllKeys(pattern) {
  ...
  try {
    ...
    log.error(`Cache getAllKeys error for pattern "${pattern}"`, { message: err.message });
    return [];
  }
}
