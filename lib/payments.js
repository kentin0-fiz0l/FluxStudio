    ...
    try {
      ...
      if (userId) {
        ...
        try {
          ...
          if (userRow.rows[0]) {
            es.sendSubscriptionConfirmedEmail(userRow.rows[0].email, userRow.rows[0].name || 'there', 'Pro').catch(() => {});
          }
        } catch { /* email service may not be available */ }
      }
    ...
    try {
      ...
      if (userResult.rows[0]) {
        ...
        try {
          ...
          if (userRow.rows[0]) {
            es.sendSubscriptionCancelledEmail(userRow.rows[0].email, userRow.rows[0].name || 'there').catch(() => {});
          }
        } catch { /* email service may not be available */ }
      }
    ...
    try {
      ...
      try {
        ...
        if (userResult.rows[0]) {
          ...
          es.sendRenewalReceiptEmail(userResult.rows[0].email, userResult.rows[0].name || 'there', amount).catch(() => {});
        }
      } catch { /* email service may not be available */ }
  ...
  async handleInvoicePaymentFailed(invoice) {
    try {
      ...
      if (userResult.rows[0]) {
        ...
        try {
          const { emailService: es } = require('./email/emailService');
          es.sendPaymentFailedEmail(user.email, user.name || 'there').catch(() => {});
        } catch { /* email service may not be available */ }
      }
    ...
    try {
      ...
      if (userId) {
        ...
        try {
          ...
          const userRow = await query('SELECT email, name FROM users WHERE id = $1', [userId]);
          if (userRow.rows[0]) {
            es.sendSubscriptionConfirmedEmail(userRow.rows[0].email, userRow.rows[0].name || 'there', planId === 'team' ? 'Team' : 'Pro').catch(() => {});
          }
        } catch { /* email service may not be available */ }
      }
