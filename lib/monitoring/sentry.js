...
try {
  ...
  log.warn('Sentry profiling module not available - profiling disabled');
...
function initSentry(app) {
  ...
  try {
    Sentry.init({
      ...
      // Performance Monitoring
      // Sample rate 0.5 captures 50% of events in production (up from 0.1)
      // This provides better error visibility while balancing quota usage
    ...
    log.info('Sentry initialized successfully', {
      ...
      release: `fluxstudio-auth@${process.env.npm_package_version || 'unknown'}`
    });
  } catch (error) {
