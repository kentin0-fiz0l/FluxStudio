...
const log = createLogger('AccountDeletor');
...
async function requestDeletion(userId, reason = null) {
  ...
  log.info('Deletion requested', { userId, gracePeriodEnds: gracePeriodEnds.toISOString() });
...
async function executeHardDelete(userId) {
  log.info('Executing hard delete', { userId });
  ...
  log.info('Hard delete completed', { userId });
}
