...
const connectionConfig = (() => {
  ...
  if (!process.env.DATABASE_URL) {
    // Don't crash the server - warn and continue without database
    // This allows the server to start and serve health checks while
    // database-dependent endpoints return appropriate errors
    log.warn('DATABASE_URL not set - database features will be disabled. Set USE_DATABASE=false to suppress this warning');
    return null;
  }
  ...
  // Override SSL configuration for DigitalOcean databases (accept self-signed certs)
  if (process.env.NODE_ENV === 'production' || config.connectionString.includes('digitalocean.com')) {
...
if (process.env.NODE_ENV === 'production' && pool) {
  log.info('Database pool created with SSL configuration', { rejectUnauthorized: false });
} else if (!pool) {
  log.info('Database disabled (USE_DATABASE=false)');
}
...
async function query(text, params) {
  ...
  try {
    ...
    log.error('Database query error', error, { text });
    throw error;
  }
}
