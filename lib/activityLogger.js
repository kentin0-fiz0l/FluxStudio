...
const log = createLogger('ActivityLogger');
...
async function log({
  ...
  try {
    ...
    if (!userId) {
      log.warn('Activity log skipped: userId is required');
      return null;
    }

    if (!type || !action) {
      log.warn('Activity log skipped: type and action are required');
      return null;
    }
...
async function getProjectActivities(projectId, options = {}) {
  try {
    ...
    log.error('Error getting project activities', error);
    return [];
  }
}
...
async function getUserActivities(userId, options = {}) {
  try {
    ...
    log.error('Error getting user activities', error);
    return [];
  }
}
...
async function getOrganizationActivities(organizationId, options = {}) {
  try {
    ...
    log.error('Error getting organization activities', error);
    return [];
  }
}
