-- Migration 011: Add HLS Streaming Support (TEXT ID Version)
...
COMMENT ON COLUMN transcoding_jobs.status IS 'Job status: pending, processing, completed, failed, canceled';
COMMENT ON COLUMN transcoding_jobs.input_url IS 'URL to source video in DigitalOcean Spaces';
COMMENT ON COLUMN transcoding_jobs.output_prefix IS 'Spaces path prefix for HLS output (e.g. hls/file-id/)';
COMMENT ON COLUMN transcoding_jobs.manifest_url IS 'URL to generated HLS master manifest';
COMMENT ON COLUMN transcoding_jobs.progress IS 'Transcoding progress percentage (0-100)';
...
-- If you have a specific app user, uncomment and adjust:
...
-- GRANT SELECT, INSERT, UPDATE ON files TO fluxstudio_app;
