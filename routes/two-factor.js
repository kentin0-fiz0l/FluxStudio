 ...
 */
router.post('/verify-setup', authenticateToken, zodValidate(twoFactorCodeSchema), asyncHandler(async (req, res) => {
  if (!authenticator) {
    return res.status(503).json({ success: false, error: '2FA service not available', code: 'TWO_FACTOR_SERVICE_UNAVAILABLE' });
  }
 ...
 */
router.post('/disable', authenticateToken, zodValidate(twoFactorCodeSchema), asyncHandler(async (req, res) => {
  if (!authenticator) {
    return res.status(503).json({ success: false, error: '2FA service not available', code: 'TWO_FACTOR_SERVICE_UNAVAILABLE' });
  }
 ...
 */
router.post('/verify', zodValidate(twoFactorVerifySchema), asyncHandler(async (req, res) => {
  if (!authenticator) {
    return res.status(503).json({ success: false, error: '2FA service not available', code: 'TWO_FACTOR_SERVICE_UNAVAILABLE' });
  }
