...
router.post('/queue', printRateLimit, checkFluxPrintEnabled, authenticateToken, asyncHandler(async (req, res) => {
...
router.get('/jobs/history', asyncHandler(async (req, res) => {
  const limit = parseInt(req.query.limit) || 100;
  const history = await printJobLogger.getJobHistory(limit);
  res.json(history);
}));

router.get('/jobs/history/filter', authenticateToken, asyncHandler(async (req, res) => {
  const userId = req.user.id;
...
router.post('/jobs/:jobId/link', zodValidate(printJobLinkSchema), asyncHandler(async (req, res) => {
  const { jobId } = req.params;
...
router.patch('/jobs/:jobId/status', zodValidate(printJobStatusSchema), asyncHandler(async (req, res) => {
  const { jobId } = req.params;
...
router.post('/jobs/sync/:fluxprintQueueId', zodValidate(printJobSyncSchema), asyncHandler(async (req, res) => {
...
router.get('/projects/:projectId/stats/detailed', authenticateToken, asyncHandler(async (req, res) => {
  const { projectId } = req.params;
  const userId = req.user.id;
...
router.post('/files/:filename/link', authenticateToken, zodValidate(printFileLinkSchema), asyncHandler(async (req, res) => {
...
router.delete('/files/:filename/link', authenticateToken, asyncHandler(async (req, res) => {
  const { filename } = req.params;
  const { project_id } = req.query;
  const userId = req.user.id;
...
router.post('/quick-print', printRateLimit, csrfProtection, authenticateToken, zodValidate(quickPrintSchema), asyncHandler(async (req, res) => {
...
router.post('/estimate', authenticateToken, zodValidate(printEstimateSchema), asyncHandler(async (req, res) => {
