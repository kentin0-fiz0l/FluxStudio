...
// Helper to create and emit notification via the notification service
async function createNotification(userId, data) {
  try {
    await notificationService.createSystemNotification({
      ...
      title: data.title,
      body: data.message,
      metadata: { type: data.type, source: 'connectors' },
    });
 ...
 */
router.get('/:provider/auth-url', authenticateToken, asyncHandler(async (req, res) => {
 ...
 */
router.get('/:provider/status', authenticateToken, asyncHandler(async (req, res) => {
  const { provider } = req.params;
 ...
 */
router.get('/:provider/files', authenticateToken, asyncHandler(async (req, res) => {
  const { provider } = req.params;
 ...
 */
router.post('/:provider/import', authenticateToken, zodValidate(connectorImportSchema), asyncHandler(async (req, res) => {
  const { provider } = req.params;
 ...
 */
router.post('/files/:fileId/link', authenticateToken, zodValidate(connectorLinkSchema), asyncHandler(async (req, res) => {
 ...
 */
router.post('/:provider/sync', authenticateToken, asyncHandler(async (req, res) => {
  const { provider } = req.params;
 ...
 */
router.post('/:provider/refresh', authenticateToken, asyncHandler(async (req, res) => {
  const { provider } = req.params;
