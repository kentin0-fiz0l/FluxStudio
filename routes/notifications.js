 ...
 */
router.put('/preferences', authenticateToken, zodValidate(updateNotificationPreferencesSchema), validateInput.sanitizeInput, asyncHandler(async (req, res) => {
  const userId = req.user.id;
 ...
 */
router.post('/projects/:projectId/read-all', authenticateToken, asyncHandler(async (req, res) => {
  const userId = req.user.id;
 ...
 */
router.get('/projects/:projectId', authenticateToken, asyncHandler(async (req, res) => {
  const userId = req.user.id;
  const { projectId } = req.params;
  const limit = Math.min(parseInt(req.query.limit, 10) || 50, 100);
  const offset = parseInt(req.query.offset, 10) || 0;
